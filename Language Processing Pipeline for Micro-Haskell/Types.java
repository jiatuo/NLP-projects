...
class MH_Type_Impl implements MH_TYPE {
    ...
    public boolean equals(MH_TYPE other) {
        return ((this.isInteger() && other.isInteger()) || (this.isBool() && other.isBool()) || (this.isArrow()
                && other.isArrow() && this.left().equals(other.left()) && this.right().equals(other.right())));
    }
    ...
    static MH_TYPE convertType(TREE tree) {
        if (tree.getChildren()[1].getRhs() == MH_Parser.epsilon) { // Case if TypeOps is empty
