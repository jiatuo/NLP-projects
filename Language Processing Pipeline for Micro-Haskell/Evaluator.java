...
class Evaluator {
	...
	static MH_EXP subst(MH_EXP e, String v1, MH_EXP e1) throws RuntimeError {
		...
		if (e.isVAR()) {
			...
			return new MH_Exp_Impl(subst(e.first(), v1, e1), subst(e.second(), v1, e1));
		} else if (e.isINFIX()) {
			return new MH_Exp_Impl(subst(e.first(), v1, e1), e.infixOp(), subst(e.second(), v1, e1));
		} else if (e.isIF()) {
			return new MH_Exp_Impl(subst(e.first(), v1, e1), subst(e.second(), v1, e1), subst(e.third(), v1, e1));
		} else if (e.isLAMBDA()) {
			String v2 = e.value();
			if (v2.equals(v1))
				return e;
	...
	static MH_EXP reduce(MH_EXP e, MH_Exp_Env env) throws RuntimeError, UnknownVariable {
		if (e.isVAR()) {
			...
			if (reducible(e1)) {
				...
				case "-":
					return new MH_Exp_Impl("NUM", v1.subtract(v2).toString());
				case "==":
					return new MH_Exp_Impl("BOOLEAN", (v1.equals(v2) ? "True" : "False"));
				case "<=":
					return new MH_Exp_Impl("BOOLEAN", (v1.compareTo(v2) <= 0 ? "True" : "False"));
				default:
					throw new RuntimeError();
				}
			}
	...
	static MH_EXP evaluate(MH_EXP e, MH_Exp_Env env) throws RuntimeError, UnknownVariable {
		MH_EXP d = e;
		while (reducible(d)) {
			d = reduce(d, env);
		}
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			LEX_TOKEN_STREAM MH_Lexer = new CheckedSymbolLexer(new MH_Lexer(fileReader));
		...
		if (runEnv != null) {
			...
			while (0 == 0) {
				...
				try {
					...
					LEX_TOKEN_STREAM lineLexer = new CheckedSymbolLexer(new MH_Lexer(lineReader));
					TREE dec = MH_Typechecker.MH_Parser.parseTokenStreamAs(lineLexer, "#TermDecl");
